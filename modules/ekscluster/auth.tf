...
locals {
  ...
  map_codebuild_roles = [
    {
      rolearn : "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${var.namespace}-${var.stage}-${var.name}-app"
      username : "codebuild-app"
      groups : ["system:masters"]
    },
    {
      rolearn : "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${var.namespace}-${var.stage}-${var.name}-infra"
      username : "codebuild-infra"
      groups : ["system:masters"]
    },
    {
      rolearn : "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${var.namespace}-${var.stage}-${var.name}-workers"
      username : "eks-nodes"
      groups : ["system:masters"]
    }
  ]
...
resource "kubernetes_config_map" "aws_auth_ignore_changes" {
  ...
  data = {
    mapRoles    = yamlencode(distinct(concat(local.map_worker_roles, var.map_additional_iam_roles, local.map_codebuild_roles)))
